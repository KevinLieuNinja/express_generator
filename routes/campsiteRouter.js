    ...
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        ...
        if (campsite && theComment) {
          // how to find the author of the comment and allows them to update it
          if (req.user._id.equals(theComment.author._id)) {
            ...
            err = new Error(
              `cant update ${req.params.campsiteId} your not the author`
            );
            err.status = 404;
            return next(err);
          }
        ...
        .then((campsite) => {
          ...
          if (campsite && theComment) {
            if (req.user._id.equals(theComment.author._id)) {
              ...
              err = new Error(
                `cant delete ${req.params.campsiteId} your not the author`
              );
              err.status = 404;
              return next(err);
            }
